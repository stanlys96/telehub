...
import { BotSearchComponent } from "@/src/components/BotSearchComponent";
...
export default function Home() {
  ...
  const filterByCategory = (attr: any) => {
  ...
  const onChangeCategory: GetProp<typeof Checkbox.Group, "onChange"> = (
    checkedValues
  ) => {
    setSelectedCategories(checkedValues as string[]);
  };

  const onChangeRating: GetProp<typeof Checkbox.Group, "onChange"> = (
    checkedValues
  ) => {
    setSelectedRatings(checkedValues as string[]);
  };

  const subcategoriesOptions = subcategoriesResult?.map((data: any) => ({
